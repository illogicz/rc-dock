...
class DockLayout extends DockPortalManager {
    ...
    dockMove(source, target, direction, floatPosition) {
        ...
        if (source === target) {
            if (Utils_1.isPanel(source) || Utils_1.isTab(source)) {
        ...
        if (direction === 'float') {
            const newSource = Utils_1.isTab(source) ? Algorithm.converToPanel(source) : source;
            ...
            if (Utils_1.isTab(source) || Utils_1.isPanel(source)) {
            ...
            if (Utils_1.isBox(source)) {
                console.warn("cannot move boxes yet");
            }
            else {
                if (Utils_1.isPanel(targ)) {
                    // pandel target
                    if (direction === 'middle') {
    ...
    render() {
        ...
        return (React.createElement("div", { ref: this.getRef, className: "dock-layout", style: style },
            React.createElement(DockData_1.DockContextProvider, { value: this },
                React.createElement(DockBox_1.DockBox, { size: Math.random(), boxData: layout.dockbox }),
